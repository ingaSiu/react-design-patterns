...
function ProductsList({ filter }) {
  const filtered = useMemo(() => {
    ...
    return PRODUCTS.filter((p) => p.toLowerCase().includes(filter.toLowerCase())).slice(0, 200);
  }, [filter]);
  ...
  return (
    <div style={{ padding: 20, fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ marginBottom: 10 }}>
        ...
        />
        <span style={{ marginLeft: 12 }}>{text === deferredText ? 'Results up-to-date' : 'Results are deferred'}</span>
      </div>
