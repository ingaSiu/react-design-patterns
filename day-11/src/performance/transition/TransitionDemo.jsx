...
function HeavyList({ query }) {
  const filtered = useMemo(() => {
    ...
    const res = HEAVY_LIST.filter((item) => item.toLowerCase().includes(query.toLowerCase()));
...
export default function TransitionDemo() {
  ...
  // startTransition tells React: this particular update is not urgent
  // keep UI responsive while you are doing everything that is defined inside startTransition function
  ...
  return (
    <div style={{ padding: 20, fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ marginBottom: 10 }}>
        ...
        <span style={{ marginLeft: 12 }}>{isPending ? 'Updating results…' : 'Idle'}</span>
      </div>
